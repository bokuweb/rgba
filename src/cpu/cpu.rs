        ...
        T: BusAccessor,
    {
        ...
        let pipeline_status = {
            match instruction {
                ...
                arm::Instruction::MRS(dec) => exec_mrs(dec, &mut self.gpr, &mut self.cpsr, &mut self.spsr)?,
                arm::Instruction::MSR(dec) => exec_msr(dec, &mut self.gpr, &mut self.cpsr, &mut self.spsr)?,
        ...
        T: BusAccessor,
    {
        ...
        let pipeline_status = {
            match instruction {
                ...
                thumb::Instruction::LSL(dec) => exec_thumb_lsl(bus, dec, &mut self.gpr, &mut self.cpsr)?,
