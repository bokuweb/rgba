        ...
        T: BusAccessor,
    {
        ...
        let (cycle, pipeline_status) = {
            match instruction {
                ...
                arm::Instruction::MRS(dec) => exec_arm_mrs(dec, &mut self.gpr, &mut self.cpsr, &mut self.spsr)?,
                arm::Instruction::MSR(dec) => exec_arm_msr(dec, &mut self.gpr, &mut self.cpsr, &mut self.spsr)?,
